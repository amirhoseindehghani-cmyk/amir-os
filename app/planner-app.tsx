...
import { Activity, ArrowRight, ArrowUp, ArrowUpDown, Brain, CalendarDays, Check, ChevronDown, ChevronLeft, ChevronRight, CircleAlert, Clock, Download, Flag, Gauge, ListTodo, Lock, MapPin, MoreHorizontal, Play, Plus, RefreshCw, RotateCcw, Search, Settings2, Sparkles, Target, Trash2, Upload, X } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Dialog, DialogContent, DialogDescription, DialogFooter, DialogHeader, DialogTitle } from '@/components/ui/dialog';
import { Progress } from '@/components/ui/progress';
import { Textarea } from '@/components/ui/textarea';
import { Input } from '@/components/ui/input';
import type { CalendarEvent, Category, Goal, OngoingTask, PlanProposal, PlannerApiResponse, PlannerDocument, Priority, ProposalChange, Review, Session, TargetUnit, WeekRecord, WeeklyTarget } from '@/lib/planner-types';
...
function weekBadge(metrics: ReturnType<typeof targetMetrics>, selectedDate: string): readonly [string, string] | null { if (metrics.done >= metrics.target) return ['Done', 'good']; const weekStart = startOfIsoWeek(selectedDate); const dayIndex = Math.round((parseLocalDate(selectedDate).getTime() - parseLocalDate(weekStart).getTime()) / 86400000); const pace = (dayIndex + 1) / 7; const pct = metrics.target > 0 ? metrics.done / metrics.target : 1; if (pct >= pace) return pct >= 1 ? ['Done', 'good'] : ['On track', 'good']; if (dayIndex <= 1) return null; if (dayIndex <= 3) return pct < pace * 0.5 ? ['Behind', 'warn'] : null; return pct < 0.3 ? ['At risk', 'bad'] : ['Behind', 'warn']; }
...
export function PlannerApp() {
  ...
  const toggleSession = (session: Session) => setDoc(current => ({ ...current, sessions: current.sessions.map(item => item.id === session.id ? { ...item, status: session.status === 'done' ? 'planned' : 'done', source:'user' as const, locked:true, lockedReason:'user-edited' as const } : item), history: [{ id: `history-${crypto.randomUUID()}`, at: new Date().toISOString(), type: session.status === 'done' ? 'session_reopened' : 'session_completed', note: session.title, weekId: startOfIsoWeek(session.date) }, ...current.history] }));
  const skipSession = (session: Session) => setDoc(current => { const unskip = session.status === 'skipped'; return { ...current, sessions: current.sessions.map(item => item.id === session.id ? { ...item, status: unskip ? 'planned' : 'skipped', source:'user' as const, locked:true, lockedReason:'user-edited' as const } : item), skippedSlots: unskip ? current.skippedSlots.filter(sl => !(sl.date === session.date && sl.startTime === session.start)) : [...current.skippedSlots, { date: session.date, startTime: session.start, endTime: endTime(session.start, session.duration), goalId: session.goalId, skippedAt: new Date().toISOString() }], history: [{ id: `history-${crypto.randomUUID()}`, at: new Date().toISOString(), type: unskip ? 'session_unskipped' : 'session_skipped', note: session.title, weekId: startOfIsoWeek(session.date) }, ...current.history] }; });
  const removeSession = (id: string) => { if (removeConfirm !== id) { setRemoveConfirm(id); return; } setDoc(current => { const removed = current.sessions.find(s => s.id === id); return { ...current, sessions: current.sessions.filter(s => s.id !== id), skippedSlots: removed ? [...current.skippedSlots, { date: removed.date, startTime: removed.start, endTime: endTime(removed.start, removed.duration), goalId: removed.goalId, skippedAt: new Date().toISOString() }] : current.skippedSlots, history: [{ id: `history-${crypto.randomUUID()}`, at: new Date().toISOString(), type: 'session_removed', note: removed?.title ?? '', weekId: startOfIsoWeek(selectedDate) }, ...current.history] }; }); setRemoveConfirm(null); };
  ...
  function saveEditSession() { if (!editSession || !editTitle.trim()) return; setDoc(current => ({ ...current, sessions: current.sessions.map(s => s.id === editSession.id ? { ...s, title: editTitle.trim(), start: editTime, duration: Number(editDuration), category: editCategory, kind: editKind, source:'user' as const, locked:true, lockedReason:'user-edited' as const } : s) })); setEditOpen(false); setEditSession(null); }
  function addSession() { if (!newTitle.trim()) return; setDoc(current => ({ ...current, sessions: [...current.sessions, { id: `session-${crypto.randomUUID()}`, date: selectedDate, start: newTime, duration: Number(newDuration), title: newTitle.trim(), category: newCategory, kind: newFixed ? 'fixed' : 'flexible', status: 'planned', source:'user' as const, locked:true, lockedReason:'user-added' as const }] })); setNewTitle(''); setAddOpen(false); }
...
function WeekStrip({ doc, week, selectedDate, onOpenWeek }: { doc: PlannerDocument; week: WeekRecord; selectedDate: string; onOpenWeek: () => void }) {
  return <div className="week-strip">
    <div className="week-strip-header"><div className="section-label"><Gauge size={14} /> Week targets</div><button className="text-button" onClick={onOpenWeek}>Full week <ArrowRight /></button></div>
    <div className="week-strip-rows">{week.targets.map(target => { const metrics = targetMetrics(doc, week, target), badge = weekBadge(metrics, selectedDate), pct = Math.min(100, metrics.done / Math.max(1, metrics.target) * 100), color = cat(target.category).dot; return <div key={target.id} className="week-strip-row"><span className="cat-dot" style={{ background: color }} /><span className="week-strip-label" style={{ color }}>{target.label}</span><div className="week-strip-bar"><div className="week-strip-fill" style={{ width: `${pct}%`, background: color }} /></div><span className="week-strip-stat">{metrics.done}/{metrics.target} {target.unit}</span>{badge && <em className={badge[1]}>{badge[0]}</em>}</div>; })}</div>
  </div>;
}
...
function DayView({ doc, setDoc, localToday, selectedDate, sessions, active, next, focusMinutes, fixedMinutes, onDate, onMorning, onReview, existingReview, onReplan, onToggle, onSkip, onEdit, onRemove, removeConfirm, onAdd, onReasoning, reasoning, onOpenWeek }: { doc: PlannerDocument; setDoc: Dispatch<SetStateAction<PlannerDocument>>; localToday: string; selectedDate: string; sessions: Session[]; active?: Session; next?: Session; focusMinutes: number; fixedMinutes: number; onDate: (date: string) => void; onMorning: () => void; onReview: () => void; existingReview?: Review; onReplan: () => void; onToggle: (session: Session) => void; onSkip: (session: Session) => void; onEdit: (session: Session) => void; onRemove: (id: string) => void; removeConfirm: string | null; onAdd: () => void; onReasoning: () => void; reasoning: boolean; onOpenWeek: () => void }) {
  ...
  const atRiskTargets = week?.targets.filter(t => t.completedValue !== undefined) ?? [];
  return <div><DateNavigator selectedDate={selectedDate} localToday={localToday} onDate={onDate} /><div className="day-layout"><section className="day-main">
    ...
    {week && <WeekStrip doc={doc} week={week} selectedDate={selectedDate} onOpenWeek={onOpenWeek} />}
    {atRiskTargets.length > 0 && <div className="at-risk-banner"><CircleAlert size={16} /><div><b>{atRiskTargets.length} target{atRiskTargets.length > 1 ? 's' : ''} at risk this week</b><span>{atRiskTargets.map(t => t.label).join(', ')}</span></div></div>}
...
function Diff({ change }: { change: ProposalChange }) { const names: Record<ProposalChange['action'], string> = { add: 'Add', remove: 'Remove', move: 'Move', shorten: 'Resize', 'update-goal': 'Priority', 'update-target': 'Weekly target', 'add-commitment': 'Fixed commitment', 'set-week-plan': 'Week plan', 'rebalance-week': 'Rebalance', 'flag-at-risk': 'At risk' }; return <article className={`proposal-change ${change.action}`}><div className="change-type">{names[change.action]}</div><div><h3>{change.label}</h3><p>{change.from && <><span>{change.from}</span><ArrowRight /></>}<span>{change.to || (change.session ? `${change.session.date} · ${change.session.start}–${endTime(change.session.start, change.session.duration)}` : '')}</span></p></div></article>; }
...
function GoalsView({ doc, setDoc }: { doc: PlannerDocument; setDoc: Dispatch<SetStateAction<PlannerDocument>> }) {
  ...
  const [gUnit, setGUnit] = useState<TargetUnit>('hours');
  ...
  const [mUnit, setMUnit] = useState<TargetUnit>('sessions');
  ...
  function openAddMonthly() { setEditingMonthly(null); setMLabel(''); setMTarget(0); setMUnit('sessions'); setMDialogOpen(true); }
  ...
  return <section className="page-section">
    ...
    <Dialog open={mDialogOpen} onOpenChange={setMDialogOpen}><DialogContent className="checkin-dialog"><DialogHeader className="checkin-header"><DialogTitle className="checkin-title">{editingMonthly ? 'Edit monthly goal' : 'New monthly goal'}</DialogTitle><DialogDescription className="checkin-date">{editingMonthly ? 'Update your monthly target' : 'What do you want to achieve this month?'}</DialogDescription></DialogHeader><div className="checkin-fields">
      ...
      <div className="goal-target-row"><label className="checkin-field" style={{ flex: 1 }}><span className="checkin-label">Target</span><Input type="number" value={mTarget || ''} onChange={e => setMTarget(Number(e.target.value))} className="checkin-input" placeholder="0" /></label><label className="checkin-field" style={{ flex: 1 }}><span className="checkin-label">Unit</span><select className="goal-select" value={mUnit} onChange={e => setMUnit(e.target.value as TargetUnit)}><option value="sessions">Sessions</option><option value="hours">Hours</option><option value="minutes">Minutes</option></select></label></div>
    ...
    <Dialog open={dialogOpen} onOpenChange={setDialogOpen}><DialogContent className="checkin-dialog"><DialogHeader className="checkin-header"><DialogTitle className="checkin-title">{editingGoal ? 'Edit goal' : 'New goal'}</DialogTitle><DialogDescription className="checkin-date">{editingGoal ? 'Update your goal details' : 'Define what you want to achieve'}</DialogDescription></DialogHeader><div className="checkin-fields">
      ...
      <div className="goal-target-row"><label className="checkin-field" style={{ flex: 1 }}><span className="checkin-label">Weekly target</span><Input type="number" value={gTarget || ''} onChange={e => setGTarget(Number(e.target.value))} className="checkin-input" placeholder="0" /></label><label className="checkin-field" style={{ flex: 1 }}><span className="checkin-label">Unit</span><select className="goal-select" value={gUnit} onChange={e => setGUnit(e.target.value as TargetUnit)}><option value="sessions">Sessions</option><option value="hours">Hours</option><option value="minutes">Minutes</option></select></label></div>
