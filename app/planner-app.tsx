...
export function PlannerApp() {
  ...
  return <div className="min-h-screen bg-background text-foreground">
    ...
    <Dialog open={morning} onOpenChange={setMorning}><DialogContent className="morning-modal"><DialogHeader><div className="eyebrow">Good morning</div><DialogTitle>Let’s plan a day you can actually follow.</DialogTitle><DialogDescription>Your answers update {formatDateLong(selectedDate)}. Lasting preferences still require approval.</DialogDescription></DialogHeader><label className="field">You woke up at<Input type="time" value={wake} onChange={event => setWake(event.target.value)} /></label><div className="field">Energy<div className="energy-row">{(['low', 'normal', 'high'] as const).map(level => <button className={energy === level ? 'selected' : ''} key={level} onClick={() => setEnergy(level)}>{level[0].toUpperCase() + level.slice(1)}</button>)}</div></div><label className="field">Anything unusual today?<Textarea value={unusual} onChange={event => setUnusual(event.target.value)} placeholder="Appointments, travel, poor sleep…" /></label><DialogFooter><Button className="primary-wide" onClick={() => requestPlan({ trigger: 'morning' })} disabled={aiStatus !== 'idle'}>{aiStatus === 'planning' ? 'Planning your day…' : 'Plan my day'}</Button></DialogFooter></DialogContent></Dialog>
