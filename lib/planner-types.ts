...
export type SkippedSlot = { date:string; startTime:string; endTime:string; goalId?:string; skippedAt:string };
export type Session = { id:string; date:string; start:string; duration:number; title:string; category:Category; kind:'fixed'|'flexible'|'routine'|'recovery'; status:'planned'|'done'|'skipped'; source?:'ai'|'user'; locked?:boolean; lockedReason?:'user-added'|'user-moved'|'user-edited'|'auto-lock'; goalId?:string; sourceTaskId?:string; contribution?:number; contributionUnit?:TargetUnit; runType?:'easy'|'long'|'tempo'|'intervals'|'recovery'; distanceKm?:number };
