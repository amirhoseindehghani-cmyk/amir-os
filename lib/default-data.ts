...
export function migratePlannerData(input:unknown,localDate:string):PlannerDocument{
  ...
  if(raw.version===5&&raw.profile&&Array.isArray(raw.weeks)){const doc=raw as PlannerDocument;if(!Array.isArray(doc.ongoingTasks))doc.ongoingTasks=[];if(!Array.isArray(doc.reviews))doc.reviews=[];for(const r of doc.reviews){if(!r.struggle)r.struggle=r.blocker||'';if(!r.carryForward)r.carryForward='';if(!r.reviewedAt)r.reviewedAt='';}if(!Array.isArray(doc.profile.customCategories)){const cats=new Set<string>();for(const g of doc.goals)cats.add(g.category);for(const s of doc.sessions)if(s.category)cats.add(s.category);doc.profile.customCategories=cats.size?[...cats]:['personal','work','fitness','learning'];}return ensureCurrentWeek(repairMondayMigration(doc,localDate),localDate);}
