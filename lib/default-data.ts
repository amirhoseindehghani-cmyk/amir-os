...
import type { PlannerDocument, Session, TargetUnit, WeeklyTarget } from './planner-types';

function normalizeUnit(unit:string,target:number):{unit:TargetUnit;target:number}{
  if(unit==='hours'||unit==='minutes'||unit==='sessions')return{unit,target};
  if(unit==='km')return{unit:'sessions',target:Math.max(1,Math.round(target/7))};
  return{unit:'sessions',target};
}
function normalizeSessionUnits(s:any):any{
  if(!s.contributionUnit||s.contributionUnit==='hours'||s.contributionUnit==='minutes'||s.contributionUnit==='sessions')return s;
  return{...s,contributionUnit:'sessions',contribution:s.contributionUnit==='km'?1:s.contribution};
}
...
const templates:WeeklyTarget[]=[
  ...
  {id:'w2',goalId:'g-marathon',label:'Running',category:'fitness',priority:2,target:4,unit:'sessions'},
...
export function createDefaultDocument(localDate:string):PlannerDocument{
  ...
  const sessions:Session[]=[
    ...
    {id:'run-today',date:today,start:'16:00',duration:55,title:'Easy run · 7 km',category:'fitness',kind:'flexible',status:'planned',goalId:'g-marathon',contribution:1,contributionUnit:'sessions',runType:'easy',distanceKm:7},
    ...
    {id:'intervals',date:tomorrow,start:'15:30',duration:60,title:'Intervals · 8 km',category:'fitness',kind:'flexible',status:'planned',goalId:'g-marathon',contribution:1,contributionUnit:'sessions',runType:'intervals',distanceKm:8},
  ...
  ],ongoingTasks:[],sessions,calendarEvents:[],top3:['Submit two internship applications','Finish SabzApply onboarding decisions','Complete easy run'],reviews:[],memories:[{id:'mem-1',text:'Prefer Dutch before 18:00',reason:'Recent sessions scheduled later were completed less consistently.',status:'pending'}],proposals:[],history:[],lastOpenedLocalDate:localDate};
}

function migrateV5toV6(doc:any,localDate:string):PlannerDocument{
  const normalizeTargets=(targets:any[])=>targets.map((t:any)=>{const n=normalizeUnit(t.unit,t.target);return{...t,unit:n.unit,target:n.target};});
  const result={...doc,version:6,weeklyTargetTemplates:normalizeTargets(doc.weeklyTargetTemplates),weeks:(doc.weeks??[]).map((w:any)=>({...w,targets:normalizeTargets(w.targets)})),monthlyTargets:(doc.monthlyTargets??[]).map((mt:any)=>{const n=normalizeUnit(mt.unit,mt.target);return{...mt,unit:n.unit,target:n.target};}),sessions:(doc.sessions??[]).map(normalizeSessionUnits)} as PlannerDocument;
...
export function migratePlannerData(input:unknown,localDate:string):PlannerDocument{
  ...
  if(raw.version===6&&raw.profile&&Array.isArray(raw.weeks)){const doc=raw as PlannerDocument;if(!Array.isArray(doc.ongoingTasks))doc.ongoingTasks=[];if(!Array.isArray(doc.calendarEvents))doc.calendarEvents=[];if(!Array.isArray(doc.reviews))doc.reviews=[];for(const r of doc.reviews){if(!r.struggle)r.struggle=r.blocker||'';if(!r.carryForward)r.carryForward='';if(!r.reviewedAt)r.reviewedAt='';}if(!Array.isArray(doc.profile.customCategories)){const cats=new Set<string>();for(const g of doc.goals)cats.add(g.category);for(const s of doc.sessions)if(s.category)cats.add(s.category);doc.profile.customCategories=cats.size?[...cats]:['personal','work','fitness','learning'];}return ensureCurrentWeek(repairMondayMigration(doc,localDate),localDate);}
  ...
  if(raw.version===4&&raw.profile&&Array.isArray(raw.goals)){
    ...
    delete (migrated as any).weeklyTargets; return migrateV5toV6(migrated,localDate);
  }
